...
def split_query_into_parts(user_query, api_key):
    # Corrected architecture for initializing the prompt
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "Analyze the user's query: '{query}' and break it down into three distinct sections: Visualization, Table, and Summary. "
    ...
    # Creating a processing chain that uses the prompt and the LLM
    chain = prompt | groq_llm

    # Invoke the chain with the user query as input
    divided_query = chain.invoke(
        {
            "query": user_query,  # Pass user query as 'query'
        }
    )
