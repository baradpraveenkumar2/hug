...
st.markdown("<h1 class='centered header'>CSV Agent Application with Cohere, LIDA & SQLite</h1>", unsafe_allow_html=True)
...
if file_path:
    ...
    st.markdown(f"<div class='column-names'>This chatbot, built on the AI4I 2020 Predictive Maintenance Dataset, helps predict machine failures based on operational data like temperature, speed, torque, and tool wear. The chatbot allows users to query for visualizations, tables, and summaries using natural language input. It leverages Cohere to interpret queries and uses SQLite for data storage. The chatbot includes error correction for column names and generates visualizations using the LIDA library for charts. The user experience is streamlined through Streamlit, with continuous conversation capabilities, making the system efficient for predictive maintenance tasks.</div>", unsafe_allow_html=True)
    ...
    if st.button("Submit"):
        ...
        if 'Visualization:' in divided_queries and 'Table:' in divided_queries and 'Summary:' in divided_queries:
            ...
            # Process the summary query using Cohere
            if 'Summary' in divided_queries and 'None' not in summary_query:
                ...
                prompt = f"Summarize the following data query: {summary_query}."
                
                try:
                    # Generate a summary using Cohere's language model
                    ...
                    summary_output = response.generations[0].text.strip()
